...
"""
crop images in [512,512] for fine-tuning whole network
"""
...
def gen_tfrec(lev):
    ...
    tfrecord_path = '/media/ict419/SSD/SICE/' + 'bottom_freq' + '_' + lev + '_' + config.model.tfrecord_suffix
    ...
    with tf.python_io.TFRecordWriter(tfrecord_path) as tfrecord_writer:
        ...
        subdirList = [os.path.join(p, o) for o in os.listdir(p) if os.path.isdir(os.path.join(p, o))]
        for item in range(len(subdirList)):
            ...
            if imgname == 'Label':
                ...
                for i in range(0, patch_length):
                    ...
                    sys.stdout.flush()
                    example = pack_example(img_rand_patches[i][1], label_rand_patches[i][1])
                    tfrecord_writer.write(example.SerializeToString())


def pack_example(img_b, label_b):
    """
    img_b --->  gaussian terrain [?,?,3]
    label_b --->  gaussian terrain [?,?,3]
    """
    features = {}

    h, w = img_b.shape

    features['h'] = tf.train.Feature(int64_list=tf.train.Int64List(value=[h]))
    features['w'] = tf.train.Feature(int64_list=tf.train.Int64List(value=[w]))

    '''bottom layer patches'''
    img_bot_patch = np.reshape(img_b, -1)
    features['train'] = tf.train.Feature(float_list=tf.train.FloatList(value=img_bot_patch))

    label_bot_patch = np.reshape(label_b, -1)
    features['label'] = tf.train.Feature(float_list=tf.train.FloatList(value=label_bot_patch))
